...
function Field({ icon, placeholder, type = "text", value, error, onChange }: FieldProps) {
  return (
    <div className="flex flex-col gap-2">
      <label
        className={cn(
          "flex items-center gap-4 h-[52px] px-5 rounded-[14px] bg-white border transition-all duration-150 cursor-text",
          "[&:hover:not(:focus-within)]:border-black/[0.12] [&:hover:not(:focus-within)]:drop-shadow-[0px_1px_1px_rgba(0,0,0,0.04)]",
          "focus-within:border-[#007aff] focus-within:drop-shadow-[0px_2px_1px_rgba(0,0,0,0.06)]",
          "border-black/[0.08]"
        )}
      >
...
export default function DemoModal() {
  ...
  const handleSubmit = useCallback(
    (e: React.FormEvent) => {
      ...
      setPhoneError(pe);
      if (ne || pe) return;
      // TODO: hook up to your lead submission API
      console.log("Lead:", { name: name.trim(), phone: phone.trim() });
      setSubmitted(true);
    },
    [name, phone]
  );
  ...
  if (!isOpen && !submitted) {
    // Keep in DOM for exit animation, but only if never opened
  }
